...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letter in string.ascii_uppercase :
            code = ( ( ord(letter) + shift - 65 ) % 26 ) + 65
            self.encrypting_dict.update({ letter : chr( code ) } )

         # Base 97
        for letter in string.ascii_lowercase :
            code = ( ( ord(letter) + shift - 97 ) % 26 ) + 97
            self.encrypting_dict.update({ letter : chr( code ) } )
    ...
    def __init__(self, text):
        ...
        for word in self.get_message_text().split(' ') :
            ...
            for shift in range(1, 26) :
                if self.apply_shift(shift) in self.get_valid_words() :
                    result = results.get( shift, 0 );
                    results.update({ shift : ( result + 1 ) })

        # getting maximum
        maximum = 0;
        maximum_dictionary_key = 0
        ...
        for item in results :
            ...
            if current >= maximum :
                maximum = current
                maximum_dictionary_key = item

        self.shift = maximum_dictionary_key;
