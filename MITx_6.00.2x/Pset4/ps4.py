...
def r_squared(y, estimated):
    ...
    """
    mean = np.mean(y);
    se = np.sum( [ (v-estimated[k])**2 for k,v in enumerate(y) ] )
    sm = np.sum( [ (v-mean)**2 for k,v in enumerate(y) ] )
    return  np.subtract(1, np.divide( se, sm  ) )
...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.plot(xVals, yVals, 'or', label='Data')

    for i in range( len( models ) ):
        estYVals = np.polyval( models[i], xVals )
        error = r_squared( yVals, estYVals )
        degree = len( models[ i ] ) - 1
        pylab.plot(xVals, estYVals, 'g', label='Fit of degree ' + str(degree) +', R2 = ' + str(round(error,3)))

    pylab.legend(loc='best')
    title = 'Fit of degree ' + str(degree) + ' R2 = ' + str(round(error, 3))
    pylab.title(title)
    pylab.show()
